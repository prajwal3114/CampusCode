...
import { courses, competitions } from '../data/coursesData';

export default function Competitions() {
  const { courseId } = useParams();
  const course = courses.find(c => c.id === courseId);
  const courseCompetitions = competitions[courseId] || [];
      ...
      <div className="ml-64 mt-16 p-8">
        <div className="max-w-6xl mx-auto">
          <div className="mb-8">
            ...
            <h1 className="text-4xl font-bold neon-text mb-2">Competitions</h1>
            <p className="text-gray-400">{course.name}</p>
          </div>
          
          <div className="grid grid-cols-1 gap-6">
            {courseCompetitions.map((competition) => (
                  ...
                  {competition.status === 'live' && (
                    <div className="mt-4 p-4 rounded-lg bg-green-500/10 border border-green-500/30">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-gray-400 text-sm">Ends in 45 minutes</span>
                      </div>
                    </div>
                  )}
                </div>
